...
export default function ChefPreparing() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await api.listOrders('preparing');
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        <View style={styles.flex1}>
          <Text style={styles.title}>Preparing Orders</Text>
          <Text style={styles.sub}>{orders.length} orders in progress</Text>
        </View>
        <Pressable
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          <View style={styles.emptyIcon}>
            <Ionicons name="flame-outline" size={26} color={colors.brand} />
          </View>
          <Text style={styles.emptyTitle}>Nothing on the stove</Text>
          <Text style={styles.emptySub}>No orders are being prepared right now.</Text>
        </View>
      ) : (
        <ScrollView
